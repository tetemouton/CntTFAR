...
Annual catch of yellowfin in `r cnt_lng` have averaged **tabl2**t between **minyear** and **maxyear**, representing **tabl2** \% of WCPFC-CA and **tabl2** \% of region(s) `r yft_reg_lst[[cnt_sht]]` yellowfin catch. Together, yellowfin catch inside `r cnt_lng` and by the `r cnt_lngalt` fleet outside your EEZ have accounted for an average **tabl2** \% of the WCPFC yellowfin catch. As such, the `r cnt_lng` fishery does not contribute significantly to overall regional impacts on the stock. The yellowfin stock in region(s) `r yft_reg_lst[[cnt_sht]]` has low levels of depletion in Region 6 but higher in Region 4 and overall `r cnt_lng` catch is a small percentage of the catch in these two regions. 
...
# Bigeye tuna
The most recent bigeye tuna assessment was conducted in 2023 [@Day2023_a]. The bigeye assessment used a nine region model, your EEZ is situated in region(s) `r bet_reg_lst[[cnt_sht]]` (\autoref{fig4} - top). Between `r endyear-9` and `r endyear` bigeye catch averaged **tabl2**t in the WCPFC-CA (\autoref{fig4} - middle).  An average of **tabl2**t (**tabl2**\% of WCPFC-CA catch) comes from `r bet_reg_lst[[cnt_sht]]`. It is estimated that bigeye spawning biomass in the WCPFC-CA and `r bet_reg_lst[[cnt_sht]]`, have declined by **tabl2**\% and **tabl2**\% respectively (\autoref{tab2}). The greatest impact on spawning biomass is from the associated - FAD - purse seine fisheries in the WCPO and from the longline fishery in `r bet_reg_lst[[cnt_sht]]` (\autoref{fig4} - bottom).
...
Annual catch of bigeye in `r cnt_lng` have averaged **tabl2**t between **minyear** and **maxyear**, representing **tabl2**\% of WCPFC-CA and **tabl2**\% of `r bet_reg_lst[[cnt_sht]]` bigeye catch. Together, bigeye catch inside `r cnt_lng` and by your fleet outside your EEZ have accounted for an average **tabl2**\% of the WCPFC bigeye catch. Regional catch of bigeye tuna, including those within or by `r cnt_lng`, are considered to be low if maintained at recent average levels. However, `r cnt_lng` should note that the FAD component of the regional purse seine fishery (the main fishery for skipjack tuna) catches juvenile bigeye and yellowfin tuna, and that fishery is contributing to the impact on this stock.

Bigeye tuna is one of two species targeted in the longline fishery operating in `r cnt_lng`* and is also taken in the purse seine fishery in `r cnt_lng`. The viability of the local and regional purse seine fishery is not dependent on bigeye tuna abundance. However, the prospects for long-term viability (and/or further expansion) of a longline fishery for bigeye in `r cnt_lng` will be dependant, in part, on trends in both national and regional fishing mortality on the bigeye stock (and economic and other factors not discussed here).
...
The most recent south Pacific albacore (hereafter, simply "albacore") assessment was conducted in 2021 [@Jordan2021_a].  The albacore assessment used a five region model, your EEZ is situated in `r alb_reg_lst[[cnt_sht]]` (\autoref{fig5} - top). Between `r endyear-9` and `r endyear` albacore catch averaged **tabl2**t in the WCPFC-CA (\autoref{fig5} - middle).  An average of **tabl2**t (**tabl2**\% of WCPFC-CA catch) comes from `r alb_reg_lst[[cnt_sht]]`. It is estimated that albacore spawning biomass in the WCPFC-CA and `r alb_reg_lst[[cnt_sht]]`, have declined by **tabl2**\% and **tabl2**\% respectively (\autoref{tab2}). The greatest impact on spawning biomass is from the sub-tropical longline fisheries both in the WCPO and in `r alb_reg_lst[[cnt_sht]]` (\autoref{fig5} - bottom).  
...
Annual catch of albacore in `r cnt_lng` has averaged **tabl2**t between **minyear** and **maxyear**, representing **tabl2**\% of WCPFC-CA and **tabl2**\% of `r alb_reg_lst[[cnt_sht]]` albacore catch. Together, albacore catch inside `r cnt_lng` and by the `r cnt_lngalt` fleet outside your EEZ have accounted for an average **tabl2**\% of the WCPFC albacore catch. Regional catch of albacore tuna, including those within or by `r cnt_lngalt` vessels fishing outside your EEZ, are considered sustainable at recent average levels. But ct.long should note that, deterministic projections\footnote{Estimates of risk using deterministic projections are likely to be underestimated as they do not include future uncertainties such as fluctuations in recruitment.} **Pilling2018** estimate that there is a 24\% chance of the overall stock falling below the LRP by 2045 at recent (status quo) catch and effort levels. While the stock remains in a biologically healthy state, the prospects for any future albacore targeted fishery in `r cnt_lng` will depend on local abundance, catch rates and economics. 
