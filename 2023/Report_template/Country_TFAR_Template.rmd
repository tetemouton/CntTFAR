...
This report should be used in conjunction with the Tuna Fisheries Assessment Report published annually by the SPC [@Hare2022_b]. Tuna stock assessment results are described using a number of technical terms, and refer to a number of key indicators of stock status (reference points). These terms are described in \autoref{tab1}. 
...
Annual catch of skipjack in `r cnt_lng` has averaged **tabl2**t between `r endyear-9` and `r endyear`, representing **tabl2**\% of WCPFC-CA and **tabl2**\% of **skjreg** skipjack catch. Together, skipjack catch inside `r cnt_lng` and by the `r cnt_lngalt` fleet outside your EEZ have accounted for an average **tabl2**\% of the WCPFC skipjack catch. Regional catch of skipjack tuna, including those within or by `r cnt_lng`, are considered sustainable at recent average levels. However, `r cnt_lng` should note that the FAD component of the regional purse seine fishery (the main fishery for skipjack tuna) catches juvenile bigeye and yellowfin tuna, and that fishery is contributing to the impact on these stocks.
...
Annual catch of yellowfin in `r cnt_lng` have averaged **tabl2**t between **minyear** and **maxyear**, representing **tabl2**\% of WCPFC-CA and **tabl2**\% of **yftreg** yellowfin catch. Together, yellowfin catch inside `r cnt_lng` and by the `r cnt_lngalt` fleet outside your EEZ have accounted for an average **tabl2**\% of the WCPFC yellowfin catch. As such, the `r cnt_lng` fishery does not contribute significantly to overall regional impacts on the stock. The yellowfin stock in **yftreg** has low levels of depletion in Region 6 but higher in Region 4 and overall `r cnt_lng` catch is a small percentage of the catch in these two regions. 
...
Annual catch of bigeye in `r cnt_lng` have averaged **tabl2**t between **minyear** and **maxyear**, representing **tabl2**\% of WCPFC-CA and **tabl2**\% of **betreg** bigeye catch. Together, bigeye catch inside `r cnt_lng` and by your fleet outside your EEZ have accounted for an average **tabl2**\% of the WCPFC bigeye catch. Regional catch of bigeye tuna, including those within or by `r cnt_lng`, are considered to be low if maintained at recent average levels. However, `r cnt_lng` should note that the FAD component of the regional purse seine fishery (the main fishery for skipjack tuna) catches juvenile bigeye and yellowfin tuna, and that fishery is contributing to the impact on this stock.
